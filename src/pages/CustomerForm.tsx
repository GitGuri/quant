...
interface Application {
  ...
  // NEW: Coverage Region
  coverage_region: "LOCAL" | "SADC" | "WWPF";
}
...
function CustomerFormFullPage({
  ...
  return (
    <div className={embed ? "w-full h-full flex flex-col" : "w-full h-[100vh] flex flex-col"}>
...
function CustomerForm({
  ...
  // Auto-fit page to container while keeping print perfect
  useEffect(() => {
    const handle = () => {
      if (!previewRef.current) return;
      const containerW = previewRef.current.clientWidth;
      const next = Math.min(1, containerW / A4_WIDTH_PX); // never upscale beyond 1
      setScale(next);
    };
    handle();
  ...
  // Helper: DD/MM/YYYY -> YYYY-MM-DD (or null if blank)
  const formatDate = (dateStr: string) => {
    if (!dateStr || dateStr.trim() === "") return null as unknown as string;
    const [day, month, year] = dateStr.split("/");
    if (!day || !month || !year) return dateStr; // fallback: keep as-is
    return `${year}-${month}-${day}`;
  };

  // Helper: return null for empty time strings
  const formatTime = (timeStr: string) => {
    if (!timeStr || timeStr.trim() === "") return null as unknown as string;
    return timeStr;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const applicationData: Partial<Application> = {
      ...
      total_amount: parseFloat((fd.get("total_amount") as string) || "0"),
      ...
      // NEW: read selected coverage region
      coverage_region: ((fd.get("coverage_region") as string) || "LOCAL") as Application["coverage_region"],
    };
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (name || surname) {
        family_members.push({
          ...
          date_of_birth: formatDate((fd.get(`family_${i}_dob`) as string) || "") as string,
        });
      }
    }
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (name || surname) {
        extended_family.push({
          ...
          date_of_birth: formatDate((fd.get(`ext_${i}_dob`) as string) || "") as string,
          premium: parseFloat((fd.get(`ext_${i}_premium`) as string) || "0"),
        });
      }
    }
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="w-full">
      ...
      {/* PREVIEW AREA */}
      <div ref={previewRef} className="bg-gray-100 p-3 rounded-md border print:bg-white">
        {/* Scaled wrapper; print removes transform */}
        <div
        ...
        >
          ...
          <div
            ...
            style={{ width: "300mm", minHeight: "387mm", boxShadow: "0 0 0.5rem rgba(0,0,0,.15)" }}
          >
            {/* Left vertical strip */}
            ...
            <div className="pl-[32mm] pr-4 pt-4 pb-6">
              {/* Header with Coverage ticks on the right */}
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-start gap-4">
                  <RegionTicks defaultValue={application?.coverage_region} />
                  <div className="w-28 h-10 border border-gray-400 flex items-center justify-center text-[10px]">
                    LOGO
                  </div>
                </div>
              </div>
                  ...
                  <tbody>
                    <tr>
                      {["R79", "R119", "R159", "R209", "R229", "R259", "R299"].map((v, i) => (
                        <Td key={i}>
                          <CheckRow name={`plan_single_${i}`} labelLeft="Single" valueRight={v} checked={application?.plan_options?.[`plan_single_${i}`]} />
                        </Td>
                      ))}
                    </tr>
                    <tr>
                      {["R89", "R129", "R169", "R219", "R239", "R269", "R309"].map((v, i) => (
                        <Td key={i}>
                          <CheckRow name={`plan_family_${i}`} labelLeft="Family" valueRight={v} checked={application?.plan_options?.[`plan_family_${i}`]} />
                        </Td>
                      ))}
                    </tr>
                  </tbody>
      ...
      <style>{`
        ...
        @media print {
          /* Remove preview scaling when printing */
        ...
        /* RegionTicks: show a ✓ when the hidden radio is checked. Modern :has support */
        label:has(input[type="radio"]:checked) .checkmark {
          display: block;
        }
        label:has(input[type="radio"]:checked) .checkmark::after {
          content: "✓";
          position: absolute;
          inset: -1px 0 0 0;
          font-size: 10px;
...
function Th({ children }: { children: React.ReactNode }) {
  return <th className="text-left font-semibold bg-gray-100">{children}</th>;
}
function Td({ children }: { children: React.ReactNode }) {
  return <td className="align-middle">{children}</td>;
}
function TdInput({ name, defaultValue, type = "text" }: { name: string; defaultValue?: string; type?: string }) {
  return (
            ...
            <div key={i} className="col-span-12" style={{ gridColumn: `span ${cell.span} / span ${cell.span}` }}>
              <div className="border border-gray-400 h-7 flex items-end">
                <div className="w-full">
                  ...
                  <div className="px-1">
                    {cell.custom ?? <input name={cell.name} defaultValue={cell.defaultValue} type="text" />}
                  </div>
                </div>
              </div>
            </div>
...
/** NEW: Three-way single-select tick boxes for LOCAL / SADC / WWPF */
function RegionTicks({ defaultValue }: { defaultValue?: "LOCAL" | "SADC" | "WWPF" }) {
  const opts: Array<{ label: "LOCAL" | "SADC" | "WWPF"; value: "LOCAL" | "SADC" | "WWPF" }> = [
    { label: "LOCAL", value: "LOCAL" },
    { label: "SADC", value: "SADC" },
    { label: "WWPF", value: "WWPF" },
  ];
  return (
    <div className="text-[10px]">
      <div className="font-medium mb-1 text-right">COVERAGE</div>
      <div className="flex items-center gap-3">
        {opts.map((o) => (
          <label key={o.value} className="inline-flex items-center gap-1 cursor-pointer">
            {/* radio = single-select; styled as square tick */}
            <input
              type="radio"
              name="coverage_region"
              value={o.value}
              defaultChecked={defaultValue ? defaultValue === o.value : o.value === "LOCAL"}
              className="hidden"
            />
            <span className="inline-block w-3.5 h-3.5 border border-gray-600 align-middle relative">
              <span className="checkmark hidden absolute inset-0" />
            </span>
            <span>{o.label}</span>
          </label>
        ))}
...
/* exports */
export default CustomerFormFullPage;
export { CustomerFormFullPage as CustomerForm };
