...
const API_BASE_URL = 'https://quantnow-sa1e.onrender.com'; // Ensure this matches your backend URL
...
const A4_WIDTH_PX = 210 * 3.7795275591; // ≈ 794px (kept for consistency even if not used)
  ...
  const handleSaveApplication = async (applicationData: any) => {
    ...
    try {
      ...
      const payload = {
        ...
        surname: applicationData.lastName || '',
        // coverage_region is already present from the form
      };

      // (Optional) if you don’t want to send firstName/lastName at all:
      ...
      // navigate to list/summary if desired
...
function CustomerForm({
  ...
  const formatDate = (dateStr: string) => {
    ...
    if (!day || !month || !year) return dateStr; // fallback if already formatted
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const applicationData: Partial<Application> = {
      ...
      // NEW: region (single-select radio)
      ...
      <div className="bg-gray-100 p-3 rounded-md border print:bg-white">
        ...
        >
          ...
          <div className="pl-[32mm] pr-4 pt-4 pb-6">
            {/* Header with Coverage ticks on the right next to logo */}
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="w-28 h-10 border border-gray-400 flex items-center justify-center">
  <img src={logo} alt="Company Logo" className="h-full object-contain" />
                  ...
                  <GridRows
                    rows={[
                      ...
                      [
                        ...
                        { label: 'Date of Birth', span: 3, name: 'dob', defaultValue: application?.date_of_birth as string },
                ...
                <tbody>
                  {Array.from({ length: 6 }).map((_, i) => (
                    <tr key={i}>
                      ...
                      <TdInput name={`family_${i}_dob`} defaultValue={application?.family_members?.[i]?.date_of_birth as string} />
                    </tr>
                  ))}
                </tbody>
              </table>
                    ...
                    {['R79', 'R119', 'R159', 'R209', 'R229', 'R259', 'R299'].map((v, i) => (
                      <Td key={i}>
                        <CheckRow
                          ...
                          labelLeft="Single"
                          valueRight={v}
                          checked={application?.plan_options?.[`plan_single_${i}`]}
                        />
                      </Td>
                    ))}
                  </tr>
                    ...
                    {['R89', 'R129', 'R169', 'R219', 'R239', 'R269', 'R309'].map((v, i) => (
                      <Td key={i}>
                        <CheckRow
                          ...
                          labelLeft="Family"
                          valueRight={v}
                          checked={application?.plan_options?.[`plan_family_${i}`]}
                        />
                      </Td>
                    ))}
                  </tr>
                ...
                <tbody>
                  {Array.from({ length: 4 }).map((_, i) => (
                    <tr key={i}>
                      ...
                      <TdInput name={`ext_${i}_dob`} defaultValue={application?.extended_family?.[i]?.date_of_birth as string} />
              ...
              <GridRows
                rows={[
                  ...
                  [
                    ...
                    { label: 'Deduction Date', span: 3, name: 'deduction_date', defaultValue: application?.deduction_date as string },
                    { label: 'Account Type', span: 3, name: 'account_type', defaultValue: application?.account_type },
                    { label: 'Commencement Date', span: 3, name: 'commencement_date', defaultValue: application?.commencement_date as string },
                  ],
                ]}
              />
            </Section>
              ...
              <GridRows
                rows={[
                  [
                    ...
                    { label: 'Date', span: 1, name: 'declaration_date', defaultValue: application?.declaration_date as string },
                    { label: 'Call Time', span: 2, name: 'call_time', defaultValue: application?.call_time as string },
      ...
      <style>{`
        ...
        /* RegionTicks: render ✓ when radio is checked (no :has needed) */
        input[type="radio"]:checked + .region-box::after {
          content: "✓";
          position: absolute;
        ...
        {opts.map((o) => (
          <label key={o.value} className="inline-flex items-center gap-1 cursor-pointer">
            ...
            <input
              ...
              className="hidden"
            />
            <span className="region-box inline-block w-3.5 h-3.5 border border-gray-600 align-middle relative" />
            <span>{o.label}</span>
          </label>
        ))}
      </div>
