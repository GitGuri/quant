...
function useProductSalesStats(products: Product[], isAuthenticated: boolean, messageApi: any) {
  ...
  useEffect(() => {
    ...
    // Example:
    // fetch('https://quantnow-cu1v.onrender.com/sales/bestsellers', { /* headers */ })
    //     .then(res => res.json())
...
const ProductsPage = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('https://quantnow-cu1v.onrender.com/products-services', {
        headers: {
  ...
  const handleSave = async (values: ProductFormValues) => {
    ...
    const endpoint = isNew
      ? 'https://quantnow-cu1v.onrender.com/products-services'
      : `https://quantnow-cu1v.onrender.com/products-services/${editingProduct!.id}`;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`https://quantnow-cu1v.onrender.com/products-services/${id}`, {
        method: 'DELETE',
  ...
  const handleRestock = async (values: { qty: number; purchasePrice: number }) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://quantnow-cu1v.onrender.com/products-services/${restockProduct.id}/stock`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
