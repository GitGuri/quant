...
export function KanbanBoard() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    const resp = await fetch('http://localhost:3000https://quantnow-cu1v.onrender.com/api/users', {
  ...
  const fetchTasksAndProjects = useCallback(async () => {
    ...
    try {
      ...
      // --- FIXED: Removed extra space in URL ---
      const projectsResponse = await fetch('http://localhost:3000https://quantnow-cu1v.onrender.com/api/projects', {
        headers: getAuthHeaders(),
      });
      ...
      // --- FIXED: Removed extra space in URL ---
      const tasksResponse = await fetch('http://localhost:3000https://quantnow-cu1v.onrender.com/api/tasks', {
        headers: getAuthHeaders(),
      });
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeColumn.id !== overColumn.id) {
      ...
      try {
        // --- FIXED: Removed extra space in URL ---
        const response = await fetch(`http://localhost:3000https://quantnow-cu1v.onrender.com/api/tasks/${activeTask.id}`, {
          method: 'PUT',
  ...
  const handleSaveNewTask = async (taskData: TaskFormData, initialStepsToAdd?: TaskStepFormData[]) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000https://quantnow-cu1v.onrender.com/api/tasks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
        body: JSON.stringify({
          ...taskData,
      ...
      if (taskData.progress_mode === 'steps' && initialStepsToAdd && initialStepsToAdd.length > 0) {
        ...
        const addStepPromises = initialStepsToAdd.map((step, index) =>
          ...
          (async () => {
            try {
              const stepResponse = await fetch(`http://localhost:3000https://quantnow-cu1v.onrender.com/api/tasks/${createdTaskId}/steps`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
                body: JSON.stringify({
                  title: step.title,
  ...
  const handleSubmitEdit = async (taskData: TaskFormData) => {
    ...
    try {
      // --- FIXED: Removed extra space in URL ---
      const response = await fetch(`http://localhost:3000https://quantnow-cu1v.onrender.com/api/tasks/${taskToEdit.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
        body: JSON.stringify({
          ...taskData,
  ...
  const handleDeleteTask = async (taskId: string) => {
    ...
    try {
      // --- FIXED: Removed extra space in URL ---
      const response = await fetch(`http://localhost:3000https://quantnow-cu1v.onrender.com/api/tasks/${taskId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
  ...
  const handleSaveNewProject = async (projectData: ProjectFormData) => {
    ...
    try {
      // --- FIXED: Removed extra space in URL ---
      const response = await fetch('http://localhost:3000https://quantnow-cu1v.onrender.com/api/projects', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
        body: JSON.stringify({
          ...projectData,
  ...
  const handleEditProject = async (projectData: ProjectFormData) => {
    ...
    try {
      // --- FIXED: Removed extra space in URL ---
      const response = await fetch(`http://localhost:3000https://quantnow-cu1v.onrender.com/api/projects/${activeProject.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
        body: JSON.stringify({
          ...projectData,
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      // --- FIXED: Removed extra space in URL ---
      const response = await fetch(`http://localhost:3000https://quantnow-cu1v.onrender.com/api/projects/${projectToDelete.id}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
