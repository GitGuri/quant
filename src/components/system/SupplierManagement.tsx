...
export function SupplierManagement() {
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:3000https://quantnow-cu1v.onrender.com/api/suppliers', {
        headers: {
  ...
  const handleUpdateSupplier = async (id: string, supplierData: SupplierSaveData) => {
    setLoading(true); // Indicate loading for the specific action
    try {
      const response = await fetch(`http://localhost:3000https://quantnow-cu1v.onrender.com/api/suppliers/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          ...getAuthHeaders(), // Add authorization header
