...
export function CustomerManagement() {
    ...
    const fetchApplications = useCallback(async () => {
        ...
        try {
            const headers = getAuthHeaders();
            const response = await fetch(`https://quantnow-cu1v.onrender.com/api/applications`, { headers });
    ...
    const handleCreateApplication = async (payload: ApplicationSaveData): Promise<boolean> => {
        try {
            const response = await fetch('https://quantnow-cu1v.onrender.com/api/applications', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
                body: JSON.stringify(payload)
            });
    ...
    const handleUpdateApplication = async (id: string, payload: ApplicationSaveData): Promise<boolean> => {
        try {
            const response = await fetch(`https://quantnow-cu1v.onrender.com/api/applications/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
                body: JSON.stringify(payload)
            });
